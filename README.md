...
where `c(x)` is a constraint function defining the constraint set `{x : c(x) ≥ 0}` and `r_Θ(x)` is a non-negative neural residual. Training combines a **PDE loss** (the stationary HJB variational inequality) with a **data loss** that regresses against precomputed reachability targets.

The supervision targets are generated by a **vertex-restricted tree search** over the control space: for control-affine dynamics with a convex-polytope control set, the Hamiltonian is maximized at a control vertex, so a finite-horizon search restricted to the control vertices recovers high-quality reachability labels with a GPU-parallel tree search — no PDE solver or full-control trajectory optimization required. Sampling-based MPC methods (MPPI, CEM, iCEM, random shooting) over the full control set are also included as baselines.
...
> only in *how* they optimize: the vertex-restricted methods
> (`beam_search`, `stochastic_beam_search`, `branch_and_bound`, `cem_discrete`)
> search over the control vertices, while the full-control sampling-based MPC
> baselines (`mppi`, `cem`, `icem`, `random_shooting`) sample over the full
> control set.
...
| `FC_DATA` | "Full-control" data — sampling-based MPC over the full control set | `mppi`, `cem`, `icem`, `random_shooting`                |
| `VRC_DATA` | "Vertex-restricted control" data — tree search over the control vertices (**the method of this work**) | `beam_search`, `stochastic_beam_search`, `branch_and_bound`, `cem_discrete` |
...
Supervision targets from a vertex-restricted tree search over the control vertices (or a full-control sampling-based MPC baseline), cached after the first run.
...
`beam_search`, `stochastic_beam_search`, `branch_and_bound`, and `cem_discrete` are vertex-restricted methods (`VRC_DATA`); `mppi`, `cem`, `icem`, and `random_shooting` are full-control sampling-based MPC baselines (`FC_DATA`).
...
| `cem_discrete`            | Cross-entropy method over control vertices     | `n_iter`, `elite_frac`                     |
| `mppi`                    | Model Predictive Path Integral (full-control baseline) | `sigma`, `lam`, `n_iter`           |
| `cem`                     | Cross-entropy method over the full control set | `n_iter`, `elite_frac`                     |
| `icem`                    | Improved CEM with colored-noise sampling       | `n_iter`, `elite_frac`, `noise_beta`       |
| `random_shooting`         | Uniform random control-sequence sampling       | —                                          |
